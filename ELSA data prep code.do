...
not clear why these are included as only core members eligible...
for consistency our analysis will be restricted to core members, i.e. drawn from the representative sample*/
/*drop 12 non core members*/
drop if _merge==2
...
some of the end of life cohort dont have a dodyr becuase the mort data is for end wave 5 only*/
clear
...
/*NB dodyr for those who were 99 in 2013 are grouped*/
...
/*NB dont know if they were living with spouse*/
label list SurvSp
...
/*NB no cases for cat 8# 'never worked' - it is in the label for the original var but no cases asigned*/
/*other relevant vars...
'have you ever done any paid work?' wpever - but the overlap is poor PLUS this ever worked var also exists in the other datasets //
enssec - nssec but only if different from eqarlier hse record??
...
/*recode fathers occu as in demakakos wealth paper*/
...
/*it doesnt use the broader term spouse but i cant see a question that does*/
...
/*total social support score - generate this after appending all wave files and imputing if using*/
...
/*living with spouse or partner*/ 
/*NB wording of questions is 'do you have a husband or wife with whom you live?*/
/*doesnt use the broader term spouse but i cant see a question that does*/
recode scptr (-9 -1 =.) (2 = 0), gen(livsppt)
...
replace scfrde2=0 if scfrd==2
replace scfrdf2=0 if scfrd==2
/*total social support score - generate this after appending all files and imputing if using MI*/
/************************************************/
/*HEALTH LITERACY*/
...
/*ITEMS FOR BEHAVIOURAL FACTOR*/
/*SMOKING*/
/*ever smoked - large % n/a here in this wave, i cant work out how to improve BUT very few of eol sample enter in wave 2 so not priority*/
tab HeSmk
label list HeSmk
/*currently*/ 
tab HESka
...
replace scfrde2=0 if scfrd==2
replace scfrdf2=0 if scfrd==2
/*total social support score - generate this after appending all wave files and imputing if using MI*/
/******************************************************/
/*FRAILTY INDEX*/
/*mobility difficulties*/
...
replace scfrde2=0 if scfrd==2
replace scfrdf2=0 if scfrd==2
/*total social support score - generate this after appending all wave files and imputing if using MI data*/
/************************************************/
/*FRAILTY INDEX*/
/*mobility difficulties*/
foreach var of varlist hemobwa-hemobpi{
...
in the word doc of the items -2 = Schedule Not Applicable - need to check this with natcen*/
...
/*effectively with the crude imputation we have already taken the longitudinal data into consideration*/
/*crude imputation justified based on the fact that the vars dont change longitudinally (excluding wealth and frailty) and no diff between pre and post crude sensitivity analysis*/
/*NB the default is to include variables on both side of the equation in each imputation - so we can mi frailty and wealth alongside the socint and ss vars*/
...
/*THIS ONLY RELEVENT FOR NUMERICAL VARS*/
/*
preserve
...
/*SET THE add() to 20 to reflect the proportion of missing data**/
...
/*CHECKING THAT THAT K-DENSITY PLOT IS ROUGHLY NORMAL*/
/*
